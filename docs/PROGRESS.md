...
43. `fix: cap Motor Test Up controls at 80% (servo-44c)` — changes the per-servo **Up** and combined **All Up** buttons to stop 20% short of the configured upper travel limit, with the 80% target shown directly on both controls. Normal Motion playback and the explicit full-range Sweep command are unchanged. Adds a browser source regression check for the safe commands and labels.
