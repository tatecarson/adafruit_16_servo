...
46. `feat: compact bake payloads and move pre-rolls into firmware (servo-y29)` — treats EEPROM as a deployment artifact: board slices omit editor-only metadata and default-valued fields, Motion tracks implicitly hold their final keyframe, and unsafe Sequence boundaries use `MOTION <id> PREP <ms>` without hidden bridge Motions or extra steps. Bake now blocks dangling Motion/Sequence/active-Setlist references, while pull/import hydrates editor defaults and removes deploy-only PREP annotations. Firmware recovers the PCA9685's last output (or models the installation's 100%-down rest after full power loss) and glides each local servo to keyframe zero before starting the armed Motion. Current project slices fell from 4477/4110/4766 bytes to 1549/1420/1600 bytes; full host tests and the OTA size guard pass with 2472 bytes of program-flash headroom. The two-slot rollback layout is unchanged, so a future storage redesign remains compatible.

47. `feat: add an explicit large-bake storage tier (servo-inw)` — retains the existing two-slot atomic rollback path through 4080 bytes and adds a 6000-byte overlapping single-record mode for compact libraries that outgrow it. Large mode uses the same CRC record format and a temporary heap lease for parsing, so ordinary static RAM stays unchanged; the firmware reports `storageMode`, `rollbackSafe`, and both capacity limits. The browser labels each board's tier and requires a prominent no-rollback/power-loss warning before a large bake. CRC corruption, calibration-tail isolation, large-to-small recovery, and actual Motion playback from a 5000-byte record are host-tested. Current 1549/1420/1600-byte project slices remain in rollback-safe dual mode.

## Git Commits (Bake Schema v2)

49. `feat: bake schema v2 — dense keys to reclaim the rollback-safe tier (servo-zzo)` — the deployed payload changes how it is spelled, not what it means: keyframes become positional `[atMs, value]`, structural keys become one character, and `kind`/`boardId` are dropped because a board slice implies both. Authoring in `library.json` is untouched. Measured against the real library, board 1 goes 5154 → 2846 bytes and board 3 goes 5361 → 2900 — both back under the 4080-byte line, which is the entire point: dual mode has rollback and the large tier never can. It also sidesteps servo-dm1 (under 4080 the read path uses the static scratch, so playback works today) and roughly halves the 258s bake measured in servo-vm8. Short keys are safe because `bakeFindValue` matches only at depth 0 of the window it scans, so keys need to be unique within an object *level*, not globally — that is what lets `g` be `gapMs` in an entry and `graceMs` in `schedulerConfig`, and both sides pin it because a collision inside one level would be silent. Firmware reads **both** schemas, chosen per field rather than per blob, so there is no flag day: a board can be flashed while still holding a v1 bake, keep playing it, and be re-baked later. Verification: `make -C test` all green (10 new firmware v2 tests, 50 new browser bake-v2 checks); `make -C test size` 122448 / 122880 (+432 headroom, 664 for dual-schema parsing). Hardware unrun.
