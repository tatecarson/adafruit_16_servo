...
- [x] Preview color follow-up — Arrange preview lines now match the Motion editor: servo tracks are amber/yellow and DC motor tracks are phosphor/green. The live preview showed the labeled **SERVO / DC MOTOR** legend and computed strokes `rgb(255, 182, 39)` / `rgb(75, 227, 193)`; `node test/verify_seq_arrangement.mjs` and `make -C test sim-verify` passed 19/19 and all browser simulation checks. Inline script syntax and `git diff --check` also passed.
...
- [x] `node test/verify_motor_test_safety.mjs` — 9/9 passed: both manual Up controls send and display 80%, neither sends 100%, and Down/Mid remain at 0%/50%.
- [x] `make -C test sim-verify` — all browser simulation/editor suites passed, including the new 9/9 Motor Test safety checks.
- [x] Inline `servo_controller.html` script syntax and `git diff --check` — passed.
...
- Firmware upload is not applicable: this changes only the browser commands issued by the Motor Test drawer. Normal Motion playback and **Sweep** remain unchanged.
