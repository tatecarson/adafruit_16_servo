...
| `bypassRamp` | boolean | no | DC tracks only. When `true`, the firmware applies the value immediately, skipping the built-in `motorState.ramping` smoothing. Default `false`. Reserved; effective in `schemaVersion: 2`. |
...
- DC values are written through the existing `motorSet(speed)` path, which respects `motorState.ramping` unless `bypassRamp` is set.
